...
def plotdfs_inoneplot(dfs, startdate,enddate):
    ...
    #print(col.find('_p'))
        if col.find('_p')==-1:
            ...
            dfloc1[col].plot(sharex=plt.gca(),color=colortable[i],label=col,linewidth=1,grid=True,alpha=0.35)
            
        else:          
            dfloc0[col].plot(sharex=plt.gca(),
                color=colortable[i],marker='|',label=col,grid=True,linestyle=None)
            dfloc1[col].plot(sharex=plt.gca(),
                color=colortable[i],marker='|',label=col,grid=True,linestyle=None,alpha=0.35)
        #plt.legend(loc="lower left")
        #
